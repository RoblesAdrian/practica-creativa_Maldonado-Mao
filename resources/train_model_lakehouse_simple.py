...
def main():
    ...
    parser.add_argument(
        "--model-base",
        default="s3a://warehouse/artifacts/models"
    )
    args = parser.parse_args()

    spark = build_spark()

    # Reinforce fs.s3a settings for executors
    ...
    # 1) Read training data from lakehouse
    ...
    # 3) Bucketize ArrDelay into classes
    ...
    # 4) StringIndexers for categorical columns
    index_columns = []
    ...
    # 8) Save model into MinIO via s3a
    out_path = f"{args.model_base}/rfc_flight_delays"
    model.write().overwrite().save(out_path)

    # 9) Save model metadata into Iceberg (drop & recreate to avoid broken state)
    ...
    '''
    meta.writeTo("lakehouse.model_registry.trained_models") \
        .using("iceberg") \
        .create()
    '''
